namespace MiniORM
{
    ...
    public abstract class DbContext
    {
        ...
        {
            this.connection = new DatabaseConnection(conectionString);

            this.dbSetProperties = this.DicoverDbSets();
        ...
        private void MapAllRelations()
        {
            foreach (var dbSetProperty in this.dbSetProperties)
            {
                ...
                var mapRealtionsGeneric = typeof(DbContext).GetMethod("MapRelations", BindingFlags.Instance | BindingFlags.NonPublic)
                                                            .MakeGenericMethod(dbSetType);

                var dbSet = dbSetProperty.Value.GetValue(this);

                mapRealtionsGeneric.Invoke(this, new object[] { dbSet });
            }
        }
            ...
            where TCollection : class, new()
        {
            ...
            foreach (var entity in dbSet)
            {
                ...
                var navigationEntities = navigationDbSet.Where(navigationEntity => 
                                                               primaryKey.GetValue(navigationEntity)
            ...
            where TEntity : class, new()
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                var naviagtionPrimaryKey = navigationProperty.PropertyType.GetProperties()
                                                                          .First(pi => pi.HasAttribute<KeyAttribute>());
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var naviagtionPropertyValue = ((IEnumerable<object>)navigationDbSet)
                                                        .First(currentNavigationProperty =>
                                                               naviagtionPrimaryKey.GetValue(currentNavigationProperty).Equals(foreignKeyValue));

                    navigationProperty.SetValue(entity, naviagtionPropertyValue);
                }
            }
        }
